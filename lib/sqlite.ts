...
export const save_preferences = async (preferencesData: {userId: string, languageLevel: string, commitment: string, objectives: string}) =>{
 try {
   db = await openDB()
  const stmt = await db.prepare("INSERT INTO userPreferences (userId, languageLevel, objctives, commiment) VALUES (?, ?, ?, ?)")
...
export const get_preferences = async (userId: string) =>{
  try {
    db = await openDB()
   const stmt = await db.prepare("SELECT * FROM userPreferencers WHERE userID=?")
